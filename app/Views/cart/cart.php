    ...
    <div class="text-center">
        ...
        <?php else: ?>
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <p class="mb-4">Total: $<?= number_format($totalPrice, 2) ?></p>
                    ...
                    <?php foreach ($cart as $item): ?>
                        <div class="border border-gray-200 rounded-lg p-4 flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-sm text-gray-600">Price: $<?= htmlspecialchars($item['price'] ?? '0.00') ?></p>
                                
                                <div class="mt-2 flex gap-2">
                                    <form method="post" action="<?= $basePath ?>/cart/remove" class="inline">
                                        <input type="hidden" name="ticketId" value="<?= (int)$item['ticketId'] ?>">
                                        <button class="bg-red-600 text-white px-4 py-1 rounded-lg">Remove</button>
                                    </form>
                                    <form method="post" action="<?= $basePath ?>/cart/buy" class="inline">
                                        <input type="hidden" name="ticketId" value="<?= (int)$item['ticketId'] ?>">
                                        <button class="bg-slate-950 hover:bg-blue-600 text-white py-1 px-4 rounded-lg">Buy Now</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <?php else: ?>
                    ...
                    <!-- Clear cart form (separate from main form) -->
                    <div class="mt-4">
