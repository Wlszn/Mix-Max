...
<header class="sticky top-0 z-50 bg-slate-950/90 backdrop-blur border-b border-slate-800 text-slate-100">
             ...
             <div class="flex items-center gap-8">
                 <a href="<?= $basePath ?>/" class="flex items-center gap-2">
                    ...
                    <svg class="w-8 h-8 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 0 0-2 2v3a2 2 0 1 1 0 4v3a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-3a2 2 0 1 1 0-4V7a2 2 0 0 0-2-2H5z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                     </svg>
                    <span class="text-2xl font-bold text-gray-900">Mix Max</span>
                    <span class="text-2xl font-black tracking-tight">Mix Max</span>
                 </a>
                ...
                <nav class="hidden md:flex items-center gap-2 text-sm">
                    ...
                    $navItems = [
                        ...
                        'Concerts' => $basePath . '/events?category=concert',
                        'Sports' => $basePath . '/events?category=sports',
                        'Theater' => $basePath . '/events?category=theater',
                        'Comedy' => $basePath . '/events?category=comedy'
                    ];
                    foreach ($navItems as $label => $url): ?>
                        <a href="<?= htmlspecialchars($url) ?>" class="px-3 py-2 rounded-full text-slate-300 hover:text-white hover:bg-slate-800 transition-colors"><?= htmlspecialchars($label) ?></a>
                    <?php endforeach; ?>
                 </nav>
             </div>
            ...
            <!-- Search -->
             <form action="<?= $basePath ?>/events" method="get" class="hidden lg:block flex-1 max-w-lg mx-8">
                 <div class="relative">
                    ...
                    >
                    <input type="text" name="search" placeholder="Search events, artists, venues…" class="w-full pl-10 pr-4 py-2 bg-slate-900 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                 </div>
             </form>
                ...
                <!-- Cart -->
                 <a href="<?= $basePath ?>/cart" class="relative">
                    <svg class="w-6 h-6 text-gray-700 hover:text-blue-600 transition-colors"
                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <svg class="w-6 h-6 text-slate-200 hover:text-indigo-300 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     ...
                     <?php if ($cartCount > 0): ?>
                        ...
                        <span class="absolute -top-2 -right-2 bg-indigo-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center"><?= (int)$cartCount ?></span>
                     <?php endif; ?>
                 </a>
                 ...
                 <?php if ($user): ?>
                    <!-- Logged-in user menu -->
                     <div class="relative group">
                        ...
                        <button class="flex items-center gap-2 text-sm font-medium text-slate-200 hover:text-indigo-300 transition-colors">
                            <div class="w-8 h-8 bg-indigo-500 text-white rounded-full flex items-center justify-center text-sm font-bold"><?= strtoupper(substr($user['username'], 0, 1)) ?></div>
                        ...
                        <div class="absolute right-0 mt-2 w-48 bg-slate-900 rounded-xl shadow-lg border border-slate-700 py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150">
                            <div class="px-4 py-2 border-b border-slate-700">
                             ...
                             <?php if (($user['role'] ?? '') === 'admin'): ?>
                                ...
                                <a href="<?= $basePath ?>/admin" class="block px-4 py-2 text-sm text-slate-200 hover:bg-slate-800">Admin Dashboard</a>
                             <?php endif; ?>
                            <a href="<?= $basePath ?>/logout"
                               class="block px-4 py-2 text-sm text-red-600 hover:bg-red-50">
                                Sign Out
                            </a>
                            <a href="<?= $basePath ?>/logout" class="block px-4 py-2 text-sm text-rose-400 hover:bg-rose-950/30">Sign Out</a>
                         </div>
                     </div>
                 <?php else: ?>
                    ...
                    <a href="<?= $basePath ?>/login" class="text-sm font-medium text-slate-300 hover:text-indigo-300 transition-colors">Sign In</a>
                    <a href="<?= $basePath ?>/register" class="text-sm font-semibold bg-indigo-500 hover:bg-indigo-400 text-white px-4 py-2 rounded-xl transition-colors">Register</a>
                 <?php endif; ?>
             </div>
 ...
 <?php if (!empty($flashSuccess)): ?>
    ...
    <div id="flash-banner" class="bg-emerald-950/80 border-b border-emerald-700 text-emerald-200 text-sm text-center py-2 px-4">
